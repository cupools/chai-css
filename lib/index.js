...
function chainMethodAtRule(utils) {
  return function (name, params) {
    ...
    var atRules = css.getAtRule(content, name, params);

    this.assert(!!atRules, 'expect #{this} to have atRule `' + name + '`', 'expect #{this} to miss atRule `' + name + '`');
...
function chainMethodRule(utils) {
  return function (selector) {
    var raw = utils.flag(this, 'object');
    var content = reviseRaw(raw);
    var rules = css.getRule(content, selector);

    this.assert(!!rules, 'expect #{this} to have selector `' + selector + '`', 'expect #{this} to miss selector `' + selector + '`');
